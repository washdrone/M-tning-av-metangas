        ...
        <div className="mt-10 space-y-0 sm:hidden">
          {steps.map((step, i) => (
            <div key={i} className="relative flex gap-4 pb-8 last:pb-0">
              {/* Vertical line */}
              {i < steps.length - 1 && (
                <div className="absolute left-[21px] top-[44px] bottom-0 w-px bg-gradient-to-b from-brand-600/30 to-transparent" aria-hidden="true" />
              )}
              <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-full border border-brand-600/30 bg-brand-600/10 text-sm font-bold text-brand-400">
                {i + 1}
              </div>
              <div className="pt-2">
                <h3 className="text-[15px] font-semibold text-white">{step.title}</h3>
                <p className="mt-1.5 text-sm leading-relaxed text-dark-400">{step.description}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`mt-14 hidden sm:grid gap-8 sm:grid-cols-2 ${gridCols}`}>
          {steps.map((step, i) => (
            <div key={i} className="relative text-center">
              ...
              <p className="mx-auto mt-2 text-sm leading-relaxed text-dark-400">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
