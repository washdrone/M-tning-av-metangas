...
export function BuyerPage({ page }: { page: BuyerContent }) {
  ...
  return <>
    ...
    {page.sections.map((section, i) => <section key={section.heading} className={`section-padding ${i % 2 === 0 ? 'section-darker' : ''}`}>
      <div className="container-narrow">
        <h2 className="text-2xl sm:text-3xl">{section.heading}</h2>
        <div className="prose-width space-y-4 mt-5 text-slate-300 leading-relaxed">
          {section.paragraphs.map(p => <p key={p}>{p}</p>)}
        </div>
        {section.items && <ul className="mt-7 grid gap-4 sm:grid-cols-2">{section.items.map(item => <li key={item} className="card-dark p-5 text-slate-300 leading-relaxed">{item}</li>)}</ul>}
      </div>
    </section>)}
    ...
    {!!page.sources?.length && <section className="section-padding"><div className="container-narrow">
      ...
      <p className="mt-3 text-sm text-slate-400">Text uppdaterad 10 september 2026. Kontrollera tillämplig regelversion och ert rapporteringsår inför beställning.</p>
      <ul className="mt-4 space-y-3">{page.sources.map(source => <li key={source.href}><a href={source.href} className="text-cyan-300 underline underline-offset-4">{source.title}</a></li>)}</ul>
    </div></section>}
    <section className="section-padding section-darker"><div className="container-narrow">
      <h2 className="text-2xl">Välj nästa steg</h2>
      <div className="mt-7 grid gap-4 sm:grid-cols-2">{page.related.map(item => <Link href={item.href} key={item.href} className="card-dark p-6"><h3 className="text-lg text-cyan-300">{item.title} →</h3><p className="mt-2 text-sm text-slate-300">{item.description}</p></Link>)}</div>
    </div></section>
