...
export function IndustryImage({ path, card = false }: { path: string; card?: boolean }) {
  const image = images[path]
  if (!image) return null
  return <div className={card ? 'mb-4 overflow-hidden rounded-xl' : 'container-wide pb-10 sm:pb-16'}>
    <Image
      src={image.src}
      alt={image.alt}
      width={1672}
      height={941}
      sizes={card ? '(max-width: 639px) 100vw, (max-width: 1023px) 50vw, 33vw' : '(max-width: 1279px) 100vw, 1200px'}
