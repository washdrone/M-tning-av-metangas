...
export default function HomePage() {
  return <>
    ...
    <section className="section-padding section-darker"><div className="container-wide">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        ...
        ].map(item => <Link key={item.href} href={item.href} className="card-dark p-6"><h3 className="text-xl">{item.title}</h3><p className="mt-3 text-slate-300">{item.text}</p><span className="mt-5 block text-cyan-300 text-sm">{item.label} →</span></Link>)}
      </div>
    </div></section>
    <section className="section-padding"><div className="container-wide">
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{sectors.map(item => <Link key={item.href} href={item.href} className="card-dark p-6"><h3 className="text-xl">{item.title}</h3><p className="mt-3 text-sm text-slate-300">{item.text}</p><span className="mt-5 block text-cyan-300">Se mätupplägg →</span></Link>)}</div>
      <Link className="mt-6 inline-block text-cyan-300 underline" href="/branscher">Se samtliga branscher</Link>
    </div></section>
    <section className="section-padding section-darker"><div className="container-narrow">
      <h2 className="text-2xl sm:text-3xl">Ett underlag som går att arbeta vidare med</h2>
      <p className="mt-5 max-w-3xl text-slate-300 leading-relaxed">Bestäm leveransen efter vem som ska använda resultatet. Driftteamet kan behöva en karta och en observationslista. Miljöansvariga kan behöva metodbeskrivning och underlag för vidare beräkning. Omfattning och format fastställs i förslaget.</p>
      ...
      ].map(([heading,text]) => <div className="card-dark p-6" key={heading}><h3 className="text-lg">{heading}</h3><p className="mt-3 text-sm text-slate-300">{text}</p></div>)}</div>
      <div className="mt-7 flex flex-wrap gap-6"><Link href="/tjanster/leveranser" className="text-cyan-300 underline">Läs om leveransens innehåll</Link><Link href="/case" className="text-cyan-300 underline">Se illustrativa exempeluppdrag</Link></div>
    </div></section>
    <section className="section-padding"><div className="container-narrow">
      ...
      ].map(([heading,text]) => <li key={heading}><h3 className="text-lg">{heading}</h3><p className="mt-3 text-slate-300">{text}</p></li>)}</ol>
    </div></section>
    <section className="section-padding section-darker"><div className="container-narrow">
      <h2 className="text-2xl sm:text-3xl">Tydligt vad som mäts och vad som beräknas</h2>
      <p className="mt-5 prose-width text-slate-300 leading-relaxed">Detektion, koncentration och utsläppsflöde besvarar olika frågor. Vind, drift och mätmetod påverkar tolkningen. Om underlaget ska användas i ett kontrollprogram behöver kraven stämmas av före beställning.</p>
      <div className="mt-7 flex flex-wrap gap-6"><Link href="/tjanster/metodik" className="text-cyan-300 underline">Metod och kvalitet</Link><Link href="/compliance" className="text-cyan-300 underline">Regelverk och rapporteringsbehov</Link><Link href="/blogg" className="text-cyan-300 underline">Kunskapsbank om utsläppsmätning</Link></div>
    </div></section>
